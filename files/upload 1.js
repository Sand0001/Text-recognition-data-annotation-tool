...
function composeTR(row_key, img_url, img_name, rec_img, rec_text, label) { // id, 图片URL，原图名，文字图名，文字，修改的文字
    ...
    //  "<form class='form' method='post' action='' target='exec_target'><table class='tab_show' border=2 id='"+
    // row_key + " max-width='100%'><tr><td class='row_id' ><p><input type='checkbox' name='num' value='check' max-width='100%'/>"+row_key
    // +"</p></td><td class='td_left'><ul><li><img class='photo' max-width='100%' alt='我是image图片文件' src='"
    // + ourl + "' title=''/></li><li><img class='photo' max-width='100%' alt='我是image图片文件' src='"
    // + rec_ourl + "' title=''/></li></td><td class='td_right'><ul><li>"+rec_text +""+label+
    // "</li><li><input class='text_mod' type='text' value='is't json_resp'/></li></ul></td><td width='15px'><select type='text' class='is_save'><option value='true'>有效</option><option value='false'>无效</option></select></td><td><input type='submit' value=保存 onclick='savetext("+
    // row_key +");'/></td></tr></table></form>";
    ...
    // text_rec = rec_text.replace('"', '&quot;', rec_text.replace("'", '&apos;', rec_text));
...
function savetext(id_tr) {  
    var id_form = $(".photo").get(id_tr).URL;
    // alert(id_tr);
    // console.log(id_form);
    // var img_url = $(".photo").get(id_tr*2).src;
    var img_url = $(".photo").get(id_tr*2).src;
    console.log("小图src："+img_url)
    ...
    // var json_response = {"imgurl":img_url,"text":text_mod,"is_save":is_save};
